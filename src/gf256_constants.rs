/// Multiplicative inverses for every byte-valued field element in `GF(2^8)`
/// under the irreducible polynomial `x^8 + x^4 + x^3 + x^2 + 1` (`0x11d`).
///
/// The index is the field element and the value at that index is its inverse.
/// `0x00` has no multiplicative inverse, so its entry is set to `0x00` as a
/// sentinel and must be handled separately by callers.
pub const GF256_INVERSE_TABLE: [u8; 256] = [
